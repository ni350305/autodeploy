...
(function(exports,html){
	...
	/**
	 * Return a new `Dialog` with the given (optional) `title` and `msg`.
	 * 
	 * @param {String}
	 *            title or msg
	 * @param {String}
	...
	/**
	 * Initialize a new `Dialog`. Options: - `title` dialog title - `message` a
	 * message to display Emits: - `show` when visible - `hide` when hidden
	 * 
	 * @param {Object}
	 *            options
	 * @api public
	...
	Dialog.prototype.render = function(options){
		var el = this.el , title = options.title , msg = options.message , self = this;
	...
	/**
	 * Show the dialog. Emits "show" event.
	 * 
	 * @return {Dialog} for chaining
	 * @api public
	 */
	...
	/**
	 * Hide the dialog with optional delay of `ms`, otherwise the dialog is
	 * removed immediately. Emits "hide" event.
	 * 
	 * @return {Number} ms
	 * @return {Dialog} for chaining
	 * @api public
...
(function(exports,html){
	...
	/**
	 * Show the overlay. Emits "show" event.
	 * 
	...
	/**
	 * Hide the overlay. Emits "hide" event.
	 * 
	 * @return {Overlay} for chaining
	 * @api public
	 */
...
(function(exports,html){
	...
	/**
	 * Return a new `Confirmation` dialog with the given `title` and `msg`.
	 * 
	 * @param {String}
	 *            title or msg
	 * @param {String}
	...
	/**
	 * Initialize a new `Confirmation` dialog. Options: - `title` dialog title -
	 * `message` a message to display Emits: - `cancel` the user pressed cancel
	 * or closed the dialog - `ok` the user clicked ok - `show` when visible -
	 * `hide` when hidden
	 * 
	 * @param {Object}
	 *            options
	 ...
	 * add by billy
	 */
	Confirmation.prototype.hidefooter = function(fn){
		this.el.find('.actions').hide();
		return this;
	};
	/**
	 * Render with the given `options`. Emits "cancel" event. Emits "ok" event.
	 * 
	 * @param {Object}
	 *            options
	 * @api public
	...
	Confirmation.prototype.render = function(options){
		...
		actions.find('.ok').click(function(e){
			e.preventDefault();
			self.emit('ok');
			if (self.callback(true) == false) {} else {
				self.hide();
			}
		});
	};
...
(function(exports,html){
	...
	/**
	 * Initialize a new `ColorPicker`. Emits: - `change` with the given color
	 * object
	 * 
	 * @api public
	 */
	...
	ColorPicker.prototype.spectrumEvents = function(){
		...
		function update(e){
			var offsetY = localPos(e).y , color = self.hueAt(offsetY - 4);
	...
	ColorPicker.prototype.renderSpectrum = function(options){
		var el = this.el , canvas = this.spectrum , ctx = canvas.getContext('2d') , pos = this._huePos , w = this.w * .12 , h = this.h;
		canvas.width = w;
		canvas.height = h;
	...
	ColorPicker.prototype.renderMain = function(options){
		var el = this.el , canvas = this.main , ctx = canvas.getContext('2d') , w = this.w , h = this.h , x = (this._colorPos.x || w) + .5 , y = (this._colorPos.y || 0) + .5;
		canvas.width = w;
		canvas.height = h;
...
(function(exports,html){
	...
	/**
	 * Return a new `Notification` with the given (optional) `title` and `msg`.
	 * 
	 * @param {String}
	 *            title or msg
	 * @param {String}
	...
	function type(type){
		return function(title,msg){
			return exports.notify.apply(this, arguments).type(type);
		}
	}
	/**
	...
	/**
	 * Initialize a new `Notification`. Options: - `title` dialog title -
	...
	/**
	 * Hide the dialog with optional delay of `ms`, otherwise the notification
...
(function(exports,html){
	...
	SplitButton.prototype.render = function(options){
		var options = options || {} , button = this.el.find('.button') , label = options.label;
...
(function(exports,html){
	...
	/**
	 * Initialize a new `Menu`. Emits: - "show" when shown - "hide" when hidden -
	 * "remove" with the item name when an item is removed - * menu item events
	 * are emitted when clicked
	 * 
	 * @api public
	 */
	function Menu(){
	...
	Menu.prototype.move = function(direction){
		...
		var next = prev.length ? prev[direction]() : this.el.find('li:first-child');
		if (next.length) {
	...
	/**
	 * Add menu item with the given `text` and optional callback `fn`. When the
	 * item is clicked `fn()` will be invoked and the `Menu` is immediately
	 * closed. When clicked an event of the name `text` is emitted regardless of
	...
	Menu.prototype.add = function(text,fn){
		var self = this , el = $('<li><a href="#">' + text + '</a></li>').addClass(slug(text)).appendTo(this.el).click(function(e){
			e.preventDefault();
			e.stopPropagation();
			self.hide();
			self.emit(text);
...
(function(exports,html){
	...
	/**
	 * Initialize a new `Card` with content for face `front` and `back`. Emits
	 * "flip" event.
	 * 
	 * @param {Mixed}
	 *            front
	...
	Card.prototype.render = function(options){
		var self = this , el = this.el = $(this.template);
