...
(function(exports, html) {
	 ...
	 * 
	 * Emits: - "show" when shown - "hide" when hidden - "remove" with the item
	 * name when an item is removed - * menu item events are emitted when
	 * clicked
	 * 
	 * @api public
	 */
	 ...
	 * 
	 * When the item is clicked `fn()` will be invoked and the `Menu` is
	 * immediately closed. When clicked an event of the name `text` is emitted
	 * regardless of the callback function being present.
	 * 
	 * @param {String}
	 *            text
	 * @param {Function}
	...
	Menu.prototype.remove = function(text) {
		...
		if (!item)
			throw new Error('no menu item named "' + text + '"');
